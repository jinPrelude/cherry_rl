...
class WaitingBatch:
    ...
    def store(self, actor_id, obs):
        assert (
            actor_id not in self.dict.keys()
        ), f"Try to overwrite existing key: self.dict[{actor_id}], keys: {self.dict.keys()}, time: {time.time()}"
    ...
    def get_all_ids_obs(self):
        ...
        obs_lst = []
        for actor_id in id_lst:
            obs_lst.append(self.dict[actor_id])
...
class ProcessedBatch:
    ...
    def store(self, actor_id, obs, action):
        assert (
            actor_id not in self.lst.keys()
        ), f"[Processed] Try to overwrite existing key: self.dict[{actor_id}], keys: {self.lst.keys()}"
